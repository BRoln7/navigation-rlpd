...
void PedestrianSFMPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  // teleoperated means no force calculation but only moves
  if (_sdf->HasElement("teleoperated"))
    this->sfmActor.teleoperated = _sdf->Get<bool>("teleoperated");
  else
    this->sfmActor.teleoperated = false;
  ...
  // Read in the minimum velocity of the pedestrian
  if (_sdf->HasElement("min_velocity"))
    this->sfmActor.desiredMinVelocity = _sdf->Get<double>("min_velocity");
  else
    this->sfmActor.desiredMinVelocity = this->sfmActor.desiredVelocity;
  
  // Read in the velocity decay factor
  if (_sdf->HasElement("vel_decay_factor"))
    this->sfmActor.vel_decay_factor = _sdf->Get<double>("vel_decay_factor");
  else
    this->sfmActor.vel_decay_factor = 0.0;
...
void PedestrianSFMPlugin::HandleObstacles() {
  ...
  for (unsigned int i = 0; i < this->world->ModelCount(); ++i) {
    physics::ModelPtr model = this->world->ModelByIndex(i); // GetModel(i);
    if (model->GetName() == "my_maze") {
      std::vector<physics::CollisionPtr> collisions = model->GetLink("base")->GetCollisions();
      for (auto collision : collisions) {
        ignition::math::Vector3d actorPos = this->actor->WorldPose().Pos();
        ignition::math::Vector3d relativePose = collision->RelativePose().Pos();
        ignition::math::Vector3d modelPos = model->WorldPose().Pos() + relativePose;
        std::tuple<bool, double, ignition::math::Vector3d> intersect =
            collision->BoundingBox().Intersect(modelPos, actorPos, 0.05, 8.0);
...
void PedestrianSFMPlugin::OnUpdate(const common::UpdateInfo &_info) {
  ...
  if (!t_init)
    {
      t_init = _info.simTime;
    }
  if (t_init)
  {
    double duration = (_info.simTime - *t_init).Double();
    if(duration < 10.0)
      return;
  }
  ...
  // update the desired velocity based on the duration
  // Define max and min velocity
  double max_velocity = this->sfmActor.desiredVelocity; // example value
  double min_velocity = this->sfmActor.desiredMinVelocity; // example value
  double k = this->sfmActor.vel_decay_factor; // example value for linear transformation parameter

  // Calculate desired_velocity based on simTime
  double elapsedTime = (_info.simTime - *t_init).Double() - 10.0;
  double desired_velocity = max_velocity - k * elapsedTime;
  if (desired_velocity < min_velocity)
  {
    desired_velocity = min_velocity;
  }
  // std::cout << std::fixed << std::setprecision(2);
  // std::cout << "Actor" << this->sfmActor.id << ": " << desired_velocity << std::endl;
  this->sfmActor.desiredVelocity = desired_velocity;
