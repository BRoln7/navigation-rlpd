...
class RosAgent:
    ...
    def __init__(self, agent_cls, use_pixels, use_pixel_embeddings, sim, num_stack):
        ...
        self.param_subscription = rospy.Subscriber(
            rospy.get_param("~param_topic", "/offroad_learning/params"),
            tm.TensorDict,
            self.receive_params,
        )
        self.rb_publisher = rospy.Publisher(
            rospy.get_param("~rb_topic", "/offroad_learning/rb_data"),
            tm.TensorDict,
            queue_size=100,
        )
        ...
        if self.use_pixel_embeddings:
            encoder_checkpoint = "/home/baohan/encoders/iql_encoder_checkpoint"
        ...
        if self.ackermann:
            ...
            self.action_scale = np.array([0.4, 1.0])
            self.action_bias = np.array([0.4, 0.0])
        ...
        if self.ackermann:
            ...
            self.action_publisher = rospy.Publisher(
                rospy.get_param("~action_topic", "/cmd_vel"),
                gm.Twist,
                queue_size=1,
            )
        ...
        self.amcl_init_publisher = rospy.Publisher(
            '/initialpose', gm.PoseWithCovarianceStamped, queue_size=1)
    ...
    def control_callback(self, obs):
        ...
        if self.state_machine.state == State.TELEOP_RECORD:
            ...
            # action = np.array([0.0, -2.0 * np.sign(obs['goal_relative'][0]/obs['goal_relative'][1])])
            self.publish_action(action, raw=True)
            # rospy.logwarn(f' action rec:  {action}')
        # rospy.loginfo("Offroad Mode: %s", self.state_machine.state)
        # rospy.logwarn(f' action:  {action}')
        elif self.state_machine.state == State.STARTING:
    ...
    def uninvert_sim(self):
        if self.state_machine.state == State.CRASHED and self.sim:
            self.state_reset(gzs.SetModelStateRequest(gzm.ModelState(
                    model_name='scout/',
                    pose=gm.Pose(
                        position=gm.Point(0.0, 0, 1),
                        orientation=gm.Quaternion(0, 0, 1, 0)
                    ),
                    reference_frame=self.fixed_frame_id,
            )))

            amcl_init = gm.PoseWithCovarianceStamped()
            amcl_init.header.stamp = rospy.Time.now()
            amcl_init.header.frame_id = self.fixed_frame_id
            amcl_init.pose.pose.orientation.z = 1.0
            self.amcl_init_publisher.publish(amcl_init)
