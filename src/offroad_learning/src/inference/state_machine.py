...
class InferenceStateMachine:
    def __init__(self, state_keys, action_scale, action_bias):
        ...
        self.scan_subscriber = rospy.Subscriber(
            rospy.get_param("~scan_topic", "/hokuyo/scan"),
            sm.LaserScan,
            self.receive_scan,
        )
        self.obstacle_min = 10.0
    ...
    def receive_scan(self, msg: sm.LaserScan):
        ranges = np.array(msg.ranges[220:500])
        self.obstacle_min = np.min(ranges[(ranges > 0.0)])
    ...
    def is_stuck(self, obs):
        # return len(self.positions) == self.positions.maxlen and np.all(np.linalg.norm(np.stack(self.positions) - obs["position"], axis=-1) < 0.00075)
        # print(self.obstacle_min)
        return self.obstacle_min <= 0.40
